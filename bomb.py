...
def listener(last,current):

    print(last+':'+current)

def callback(current,phase,block):

    binary=bin(phase)[2:].zfill(4)
    indexed=[str(e) for e in range(4)]
    for q,e in enumerate(current):
        indexed[q]=str(current[q+1]['value'])

    data=''.join(indexed)

    print(data+' -> '+str(binary))
    ...
    for q,e in enumerate(indexed):
        if (str(binary)[q]!=e and current[q+1]['changed']!=False):
            print('STRIKE!!!')
            strike()

def testing(current,phase):
    if current[0]!=phase[0]:
        print('STRIKE!!!')
        strike()

    if current[1]!=phase[1]:
        print('STRIKE!!!')
        strike()
...
def setup_phases():
    ...
    wires = Wires(component_wires, wires_target,testing)
    # setup the pushbutton thread
    button = Button(component_button_state, component_button_RGB, button_target, button_color, timer)
    # bind the pushbutton to the LCD GUI so that its LED can be turned off when we quit
    gui.setButton(button)
    # setup the toggle switches thread
    toggles = Toggles(component_toggles, toggles_target,callback)
    ...
    button.start()
    toggles.start()
    #Thread(target=toggles.listen,args=(listener,)).start()
...
def check_phases():
    ...
    # check the toggles
    if (toggles._running or button._running):
        # update the GUI
        gui._ltoggles["text"] = f"Toggles: {toggles.__str__()}"
        # the phase is defused -> stop the thread
        if (toggles._defused):
